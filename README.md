...
メールアプリ **[右筆（ゆうひつ）](https://apps.apple.com/jp/app/%E5%8F%B3%E7%AD%86-%E3%82%86%E3%81%86%E3%81%B2%E3%81%A4/id6799386029)** と MCP で連携し、声でメールを扱えます。

| 話しかける例 | 動作 |
|---|---|
| 新着メールある？ | 未読メールを「要返信」「重要」「あとで」「不要」に仕分けて、要点を読み上げる |
| 〇〇さんからのメールを要約して | 該当するメールを探して要約する |
| 返信の下書きを作って | 右筆に返信の下書きを作る（**送信はしない**。送信は右筆の画面で自分で行う） |

使い方:
1. 右筆の「設定 → AIエージェント」で連携を ON にする（既定は OFF）
2. AIエージェントは自動で接続します（設定 → 連携 で「右筆 (yuhitsu)」が「接続済み」になれば OK）

- 右筆の連携は、この Mac の中からしか接続できません。メールの送信・削除・転送の機能は連携に含まれていません
- AI を Claude / GPT / Gemini にしている間は、読んだメール本文が各社のサーバーに送られます。社外に出したくないメールを扱うときは、AI をローカル（Ollama）にしてください
